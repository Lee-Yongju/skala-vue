...
const handleCreate = async () => {
  ...
  try {
    const payload = { title: textInput.value, body: '샘플용 내용입니다', userId: 1 }

    const reponse = await axios.post(BASE_URL, payload)
    console.log('POST 성공', reponse.data)

    items.value.unshift(reponse.data)
    textInput.value = ''
  } catch (error) {
...
const handleUpdate = async (id) => {
  try {
    const editPayload = { title: '변조된 타이틀', body: '수정 완료', userId: 1 }
    ...
    <ul class="item-list">
      <li v-for="item in items" :key="item.id" class="item-card">
        ...
        <div class="btn-group">
          ...
          <button @click="handleDelete(item.id)" class="btn-delete">DELETE (삭제)</button>
        </div>
      </li>
    </ul>
  </div>
